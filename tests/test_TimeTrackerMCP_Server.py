...
class TestTimeTrackerMCP_Server(unittest.TestCase):
    ...
    def test_a_call_gives_synchronisation_its_turn(self):
        ...
        every tool goes through and the only moment the document has just
        been read and is owned by this thread.
        """
        if not getattr(self.mcp_server, 'SYNC_AVAILABLE', False):
            self.skipTest("the sync client is not importable here")
        with patch.object(self.mcp_server.sync_engine, 'bring_up_to_date',
                          return_value={'applied': None, 'save_error': None}) as brought, \
             patch.object(self.mcp_server.sync_engine, 'nudge') as nudged:
            tracker = self.mcp_server.get_tracker()
